...
from train_eval.trajectory_prediction.initialization import initialize_prediction_network, initialize_metric, initialize_dataset
import torch
import time
import math
import os
...
class TrainAndEvaluator:
    """
    Train/Evaler class for running train-val loops
    """
    def __init__(self, cfg: Dict, checkpoint_path=None, just_weights=False, writer=None):
        """
        ...
        train_set = initialize_dataset(cfg['dataset'], 
            encoder_type=cfg['encoder_type'], version='train')
        val_set = initialize_dataset(cfg['dataset'], 
            encoder_type=cfg['encoder_type'], version='eval')
        ...
        # Initialize model
        self.model = initialize_prediction_network(
